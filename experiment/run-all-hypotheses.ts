/**
 * Preregistered Experiment: Union-Find vs Flat Context Compaction
 *
 * Runs all three confirmatory hypotheses (H1, H2, H3) using real Gemini 3.1
 * Flash Lite API calls on real GitHub issue conversations.
 *
 * H1 (Recall): Compress conversations both ways, ask factual questions,
...
const OUT_DIR =
  '/Users/junekim/Documents/union-find-compaction-for-gemini-cli/experiment';
...
class RealSummarizer implements Summarizer {
  ...
  async summarize(messages: string[]): Promise<string> {
    this.callCount++;
    const prompt = `Summarize the following conversation messages into a concise, information-dense paragraph. Preserve all specific technical details, file paths, tool results, variable names, and user constraints.\n\nMessages:\n${messages.map((m, i) => `[${i + 1}] ${m}`).join('\n\n')}`;
...
async function judgeAnswer(
  ...
  // Judge: does the answer match ground truth?
...
async function main() {
  ...
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║  PREREGISTERED EXPERIMENT: Union-Find vs Flat Compression   ║');
  ...
  const h2AllLatencies: Array<{ convId: string; msgIdx: number; latencyMs: number; isMerge: boolean }> = [];
  ...
  for (let ci = 0; ci < conversations.length; ci++) {
    ...
    const questions = await generateQuestions(msgs, convId);
    console.log(`  Generated ${questions.length} questions`);
    for (const q of questions) {
      console.log(`    Q: ${q.question.slice(0, 80)}...`);
    }

    // Step 2: Flat compression
    ...
    // Step 3: Union-find compression (also measures H2 latency + H3 cost)
    console.log('  Running union-find compression...');
    ...
    for (let mi = 0; mi < msgs.length; mi++) {
      ...
      const start = performance.now();
      await contextWindow.append(msgs[mi]);
      const elapsed = performance.now() - start;
      const triggered = summarizer.callCount > callsBefore;
    ...
    console.log(
      `  Union-find: ${summarizer.callCount} calls, ` +
    ...
    perConvSummary.push({
      ...
      unionCalls: summarizer.callCount,
      unionMerges: summarizer.callCount,
    });
    ...
    // Throttle between conversations to avoid rate limits
  ...
  console.log('╔══════════════════════════════════════════════════════════════╗');
  console.log('║                        RESULTS                             ║');
  console.log(`║  Elapsed: ${elapsedMin} minutes, API calls: ${totalApiCalls}, tokens: ${totalApiTokens}  `);
  console.log('╚══════════════════════════════════════════════════════════════╝');
  ...
  // ─── H2: Latency ──────────────────────────────────────────────────────────
  console.log('\n═══ H2: LATENCY ═══');
  const allLat = h2AllLatencies.map((l) => l.latencyMs).sort((a, b) => a - b);
  const mergeLat = h2AllLatencies.filter((l) => l.isMerge).map((l) => l.latencyMs).sort((a, b) => a - b);
  const noMergeLat = h2AllLatencies.filter((l) => !l.isMerge).map((l) => l.latencyMs).sort((a, b) => a - b);

  console.log(`Total appends: ${allLat.length}`);
  console.log(`  p50: ${percentile(allLat, 50).toFixed(1)} ms`);
  console.log(`  p95: ${percentile(allLat, 95).toFixed(1)} ms`);
  console.log(`  p99: ${percentile(allLat, 99).toFixed(1)} ms`);
  console.log(`  Max: ${allLat[allLat.length - 1].toFixed(1)} ms`);

  if (mergeLat.length > 0) {
    console.log(`\n  With LLM merge (${mergeLat.length} appends, ${((mergeLat.length / allLat.length) * 100).toFixed(1)}%):`);
    console.log(`    p50: ${percentile(mergeLat, 50).toFixed(1)} ms`);
    console.log(`    p95: ${percentile(mergeLat, 95).toFixed(1)} ms`);
    console.log(`    Max: ${mergeLat[mergeLat.length - 1].toFixed(1)} ms`);
  }
  if (noMergeLat.length > 0) {
    console.log(`\n  Without merge (${noMergeLat.length} appends):`);
    console.log(`    p50: ${percentile(noMergeLat, 50).toFixed(1)} ms`);
    console.log(`    p95: ${percentile(noMergeLat, 95).toFixed(1)} ms`);
  }

  const h2p95 = percentile(allLat, 95);
  const h2Pass = h2p95 < 100;
  console.log(`\n  CRITERION: p95 < 100ms`);
  console.log(`  VERDICT: ${h2Pass ? 'PASS ✅' : 'FAIL ❌'} (p95 = ${h2p95.toFixed(1)} ms)`);
  ...
  console.log(`║  H2 (Latency): ${h2Pass ? 'PASS ✅' : 'FAIL ❌'}  (p95 = ${h2p95.toFixed(1)} ms)`.padEnd(63) + '║');
  ...
  const results = {
    ...
    h2: {
      totalAppends: allLat.length,
      p50: percentile(allLat, 50),
      p95: h2p95,
      p99: percentile(allLat, 99),
      max: allLat[allLat.length - 1],
      mergeAppends: mergeLat.length,
      mergeP95: mergeLat.length > 0 ? percentile(mergeLat, 95) : null,
      pass: h2Pass,
    },
    h3: {
  ...
  // Write H2 latency CSV
  const csvLines = ['conv_id,msg_idx,latency_ms,is_merge'];
  for (const l of h2AllLatencies) {
    csvLines.push(`${l.convId},${l.msgIdx},${l.latencyMs.toFixed(4)},${l.isMerge}`);
  }
  fs.writeFileSync(
    path.join(OUT_DIR, 'performance', 'union-find-real-latencies.csv'),
    csvLines.join('\n'),
  );
  console.log('Latency CSV written to experiment/performance/union-find-real-latencies.csv');
}
